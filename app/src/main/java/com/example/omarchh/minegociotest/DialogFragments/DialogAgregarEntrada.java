...
import com.example.omarchh.minegociotest.AdapterMPagoSpinner;
...
import com.example.omarchh.minegociotest.MotivoRetiroSpinnerAdapter;
...
public class DialogAgregarEntrada extends DialogFragment implements View.OnClickListener, TextWatcher {
    ...
    private byte ConfirmarProceso() {
        ...
        if (bMonto.compareTo(new BigDecimal(0)) == 0) {
            Toast.makeText(getActivity(), "Ingrese un monto mayor a cero", Toast.LENGTH_SHORT).show();
        } else {
            ...
            if (tipo.equals("Retiro")) {
                ...
                if (bMonto.compareTo(montoComparar) == 1) {
                    Toast.makeText(getActivity(), "El monto es mayor al disponible", Toast.LENGTH_SHORT).show();
    ...
    private class CargarDatos extends AsyncTask<Void, Void, Byte> {
        ...
        protected void onPostExecute(Byte aByte) {
            ...
            if (mResumenMedioPagoList.size() == 0 && tipo.equals("Retiro")) {
                Toast.makeText(getContext(), "No tiene saldo para hacer un retiro", Toast.LENGTH_SHORT).show();
                dialog.dismiss();
            }
            if (listMedioPago != null) {
                if (mResumenMedioPagoList != null) {
